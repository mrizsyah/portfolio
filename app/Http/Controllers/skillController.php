    ...
    public function update(Request $request)
    {
        if($request->method() == 'POST') {
            $request->validate([
                ...
                '_language.required' => 'Silahkan Masukan Bahasa Pemrograman yang anda kuasai',
                '_workflow.required' => 'Silahkan Masukan Workflow yang anda kuasai',
            ]);

            metadata::updateOrCreate(['meta_key'=>'_language'],
            ['meta_value' => $request->_language]);
            metadata::updateOrCreate(['meta_key'=>'_workflow'],
